...
<<echo=FALSE, results=tex>>=
latex(tab6b, title="", caption="Surgical Procedures by Diagnosis Group for Patients with Follow-up", where="!h", size="footnotesize", col.just=rep("c",2),colheads=c(snam[k], pracs[w]), longtable=TRUE, lines.page=60, file="", insert.bottom =mlCap("$x\\pm s$ represents mean$\\pm 1$ Standard Deviation for Number of Levels. \\\\ N is the number of non-missing values. \\\\  Denominator lower than 3-month accrual is because of the missing values.\\\\ A patient could possibly fall into more than one diagnosis group"))
@
\clearpage
<<echo=FALSE, results=tex>>=
myrow <- rep('', nrow(tab7))
