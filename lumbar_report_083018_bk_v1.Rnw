...
<<echo=FALSE, results=tex>>=
latex(tab6b, title="", caption="Surgical Procedures by Diagnosis Group for Patients with Follow-up", where="!h", size="footnotesize", col.just=rep("c",2), colheads=c(latexTranslate(pracs[k]), "QOD Total"), longtable=TRUE, lines.page=100, file="", insert.bottom =mlCap("$x\\pm s$ represents mean$\\pm 1$ Standard Deviation for Number of Levels. \\\\ N is the number of non-missing values. \\\\  Denominator lower than 3-month accrual is because of the missing values."))
@
\clearpage
<<echo=FALSE, results=tex>>=
myrow <- rep('', nrow(tab7))
...
## fig 2##
practice<-pracs[k]
tab_figure <- figure_construct(datas=datas)

dfp_satisfaction <- data.frame(Site= c(practice,"QOD Total",practice,"QOD Total",practice,"QOD Total",practice,"QOD Total",practice,"QOD Total"),satisfaction = c("1 Met","1 Met", "2 Not As Much As Hoped", "2 Not As Much As Hoped", "3 Would Not Undergo", "3 Would Not Undergo", "4 Same or Worse", "4 Same or Worse","Top 2 (1&2)","Top 2 (1&2)"),percent = c(round(tab_figure[1,1]),round(tab_figure[1,2]),round(tab_figure[2,1]),round(tab_figure[2,2]),round(tab_figure[3,1]),round(tab_figure[3,2]),round(tab_figure[4,1]),round(tab_figure[4,2]),round(tab_figure[5,1]),round(tab_figure[5,2])))
...
pl<-ggplot(data=dfp_satisfaction, aes(x=satisfaction, y=percent, fill=Site)) +
  ...
  scale_fill_brewer(palette="Paired")+scale_x_discrete(limits=c("1 Met", "2 Not As Much As Hoped","3 Would Not Undergo", "4 Same or Worse","Top 2 (1&2)"))+
  theme(panel.grid.major = element_line(color = "grey"),text = element_text(size=12),axis.text.x=element_text(angle=30,hjust=0.5,vjust=0.6))
...
dfp_readmission <- data.frame(Site= c(practice,"QOD Total",practice,"QOD Total",practice,"QOD Total"),readmit = c("30-day Hospital Readmission", "30-day Hospital Readmission", "3-month Hospital Readmission", "3-month Hospital Readmission", "3-month Revision Surgery", "3-month Revision Surgery"),percent = c(tab_figure[7,1],tab_figure[7,2],tab_figure[9,1],tab_figure[9,2],tab_figure[8,1],tab_figure[8,2]))


pl<-ggplot(data=dfp_readmission, aes(x=readmit, y=percent, fill=Site)) +
  scale_y_continuous(limits=c(0,10),breaks=seq(0,10,1),labels = function(x) paste0(x, "%"))+
  geom_bar(stat="identity", position=position_dodge())+
  geom_text(aes(label=percent), vjust=-0.2, color="black",hjust = 0,
            position = position_dodge(0.9), size=4)+
  labs(y="Readmission Percent",x="Readmission Category")+
  scale_fill_brewer(palette="Paired")+scale_x_discrete(limits=c("30-day Hospital Readmission", "3-month Hospital Readmission", "3-month Revision Surgery"))+
  theme(panel.grid.major = element_line(color = "grey"),text = element_text(size=12),axis.text.x=element_text(angle=30,hjust=0.5,vjust=0.6))
...
<<echo=FALSE, results=tex>>=
    ...
    latex(tab, 
          title='', 
          caption=paste("Site", ': Self Benchmark Patient Reported Ouctomes and Utilization', sep=''), 
...
<<echo=FALSE, results=tex>>=
    cat("\\begin{figure}[!htbp]",
        ...
        "\\caption[Self-benchmark: Observed vs Expected.]{", latexTranslate(pracs[k]), ": Self-benchmark: observed mean vs expected mean (as in the table above). Parenthesis denotes bootstrap 95$\\%$ CI of observed mean. Your site is statistically significantly different if triangle is outside parentheses and it is colored with red}",
        "\\end{center}",
        "\\end{figure}", sep='')
@
\end{landscape}
