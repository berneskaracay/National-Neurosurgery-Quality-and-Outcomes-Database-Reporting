...
setwd("C:\\Users\\karacb1\\Desktop\\lumbar_surgeon_report")
...
data$date_of_surgery<-NA
data$date_of_surgery=as.Date(data$date_surgery_performed, format='%Y-%m-%d')
data<-data[data$date_of_surgery>="2017-01-01" & data$date_of_surgery<= "2017-12-31",]


# extract practice/center name, patient id, surgeon id, hospital/surg_location #
n <- nrow(data)
data_surgeon<-data[,c("pt_study_id","surgeon")]
...
str(data$days.readmit)
table(data$days1,exclude = NULL)
table(data$days2,exclude = NULL)
table(data$days.readmit,exclude = NULL)
...
table(data$majoradep, exclude = NULL) 
...
table(data$majorade, exclude = NULL) 
...
##########      Patient Interview        ###################
############################################################
table(data$pt_satisfaction_index.3m,exclude = NULL)
...
table(data$pt_satisfaction_index2.3m,exclude = NULL)
...
tab9pfun <- function(datas) {
  ...
  for (i in 1:4) {
    tmp_12 <- datas[!is.na(datas[[vars1[i]]]) & !is.na(datas[[vars3[i]]]),]
    tmp_3 <- datas[!is.na(datas[[vars1[i]]]) & !is.na(datas[[vars2[i]]]),]
    tmp_base<- datas[!is.na(datas[[vars1[i]]]),]
    M[i,1] <- nrow(tmp_base)
    M[i,4] <- nrow(tmp_3)
    M[i,7] <- nrow(tmp_12)
    M[i,2] <- mean(tmp_base[[vars1[i]]])
    M[i,3] <- sd(tmp_base[[vars1[i]]])
    M[i,5] <- mean(tmp_3[[vars2[i]]])
    M[i,6] <- sd(tmp_3[[vars2[i]]])
    if (nrow(tmp_12)>0) {
      M[i,8] <- mean(tmp_12[[vars3[i]]])
      M[i,9] <- sd(tmp_12[[vars3[i]]])
    }
  }
  return(M)
}
...
plotfun1 <- function(x, yli, yma, yla, mai, yll) {
  ...
  legend(x=9, y=0,legend=c(latexTranslate(snam[k]),latexTranslate(pracs[w])), pch=c(16,17), col=c('black','red'), bty='n', xpd=NA, ncol=1)
...
for (w in seq(length(pracs))) {
  ...
  tab <- table(data1$surgeon)
  snam <- names(tab)[tab >= 20]
  for (k in seq_along(snam)) {
    Sweave("lumbar_surgeon_082218.Rnw", output=paste0(filepracs[w], '_', snam[k], ".tex"))
  }
}
...
for (w in seq(length(pracs))) {
  ...
  for (k in seq_along(snam)) {
    for (q in 1:3) {
      if(system(paste0("pdflatex -halt-on-error ", filepracs[w], "_", snam[k], ".tex")) != 0L)
        stop("Unable to compile latex document ", filepracs[w], "_", snam[k], ".tex")
    }
  }
}
